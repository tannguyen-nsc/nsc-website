...
namespace NscSoftware\Components\FeatureFlexibleContentExtension;
...
add_action('admin_enqueue_scripts', function () {
    ...
    $data = [
        'labels' => [
            'placeholder' => __('Search...', 'NscSoftware'),
            'noResults' => __('No components found', 'NscSoftware'),
        ],
    ...
    wp_localize_script('NscSoftware/assets/admin', 'FeatureFlexibleContentExtension', $data);
});
...
add_filter('acf/fields/flexible_content/layout_title', function ($title, $field, $layout, $i) {
    ...
    if (is_file($componentScreenshotPath)) {
        ...
        $newTitle = '<span class="NscSoftwareComponentScreenshot">';
        $newTitle .= '<img class="NscSoftwareComponentScreenshot-previewImageSmall" width="' . $imageSize[0] . '" height="' . $imageSize[1] . '" src="' . $componentScreenshotUrl . '" loading="lazy">';
        $newTitle .= '<span class="NscSoftwareComponentScreenshot-label">' . $title . '</span>';
        $newTitle .= '</span>';
        $title = $newTitle;
    }
    return $title;
