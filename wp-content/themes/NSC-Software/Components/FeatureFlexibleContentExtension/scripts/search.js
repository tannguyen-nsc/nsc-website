  ...
  const FeatureFlexibleContentExtension = window.acf.Model.extend({
    ...
    initialize: function () {
      ...
      document.body.addEventListener('click', (event) => {
        if (event.target.classList.contains('NSC-SoftwareComponentSearch-field')) {
    ...
    render: function ($tooltip, position) {
      ...
      if ($tooltip.querySelector('.NSC-SoftwareComponentSearch')) {
        return
      }

      $tooltip.insertAdjacentHTML(
        position === 'top' ? 'beforeend' : 'afterbegin',
        `<div class="NSC-SoftwareComponentSearch">
          <input type="text" placeholder="${placeholder}" class="NSC-SoftwareComponentSearch-field">
          <div class="NSC-SoftwareComponentSearch-noResults" hidden>${noResults}</div>
        </div>`
      )

      const $searchField = $tooltip.querySelector('.NSC-SoftwareComponentSearch-field')
      const $noResults = $tooltip.querySelector('.NSC-SoftwareComponentSearch-noResults')
