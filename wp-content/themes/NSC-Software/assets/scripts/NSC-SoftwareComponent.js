...
const nscSoftwareComponents = new WeakMap()
const parents = new WeakMap()

export default class NscSoftwareComponent extends window.HTMLElement {
  constructor () {
    super()
    let setReady
    ...
    nscSoftwareComponents.set(this, [isReady, setReady])
  }
  ...
  async connectedCallback () {
    if (hasScript(this)) {
      ...
      if (hasParent(this)) {
        const [parentLoaded] = nscSoftwareComponents.get(parents.get(this))
        await parentLoaded
      }
...
function getComponentPath (node) {
  ...
  return window.NscSoftwareData.componentsWithScript[componentName]
}
...
function setComponentReady (node) {
  const setReady = nscSoftwareComponents.get(node)[1]
  setReady()
}
