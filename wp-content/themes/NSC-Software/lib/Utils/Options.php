...
class Options
{
            ...
            foreach (static::OPTION_TYPES as $optionType => $option) {
                ...
                if (!$isTranslatable && $isCurrentPage) {
                    ...
                    add_filter(
                        ...
                        'NscSoftware\Utils\Options::getDefaultAcfLanguage',
                        101
                    );
    ...
    protected static function createOptionSubPage(string $optionType, string $optionCategory = 'Global')
    {
        if (empty(static::$optionPages[$optionType]['sub_pages'][$optionCategory])) {
            ...
            $categoryTitle = _x($optionCategory, 'title', 'NscSoftware');
    ...
    protected static function getOptionField(string $key, bool $isTranslatable)
    {
        if ($isTranslatable) {
            ...
            add_filter('acf/settings/current_language', 'NscSoftware\Utils\Options::getDefaultAcfLanguage', 100);

            $option = get_field('field_' . $key, 'option');

            remove_filter('acf/settings/current_language', 'NscSoftware\Utils\Options::getDefaultAcfLanguage', 100);
        }
