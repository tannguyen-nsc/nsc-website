...
class TimberDynamicResize
{
     ...
     * @var array
     */
    public $NscSoftwareResizedImages = [];
    ...
    public function resizeDynamic(string $src, float $w, float $h = 0, string $crop = 'default', bool $force = false)
    {
        ...
        if ($this->enabled) {
            ...
            if (empty($this->NscSoftwareResizedImages)) {
                add_action('shutdown', [$this, 'storeResizedUrls'], -1);
            }
            $this->NscSoftwareResizedImages[$w . '-' . $h . '-' . $crop] = [$w, $h, $crop];
    ...
    public function storeResizedUrls()
    {
        global $wpdb;
        $tableName = $this->getTableName();
        $values = array_values($this->NscSoftwareResizedImages);
