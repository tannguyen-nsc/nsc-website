![NSC-Software – WordPress Starter Theme for Developers](.github/assets/banner.svg 'NSC-Software – WordPress Starter Theme for Developers')

# Fall in love with WordPress (again)

[![standard-readme compliant](https://img.shields.io/badge/readme%20style-standard-brightgreen.svg?style=flat-square)](https://github.com/RichardLitt/standard-readme)
[![Build Status](https://travis-ci.org/NSC-Softwarewp/NSC-Software.svg?branch=master)](https://travis-ci.org/NSC-Softwarewp/NSC-Software)
[![Code Quality](https://img.shields.io/scrutinizer/g/NSC-Softwarewp/NSC-Software.svg)](https://scrutinizer-ci.com/g/NSC-Softwarewp/NSC-Software/?branch=master)

[NSC-Software](https://NSC-Softwarewp.com/) is a lightning-fast WordPress Starter Theme for component-based development with [ACF Pro](#advanced-custom-fields).
...
2. Change the domain variable in `NSC-Software/vite.config.js` to match your domain:  
`const wordpressHost = 'http://your-project.test'`
3. Navigate to the theme folder and run the following command in your terminal:

```
# wp-content/themes/NSC-Software
composer install
npm install
npm run build
```

4. Open the WordPress backend and activate the NSC-Software theme.
...
NSC-Software comes with a ready to use Base Style built according to our best practices for building simple, maintainable components. Go to `domain/BaseStyle` to see it in action.
...
NSC-Software uses [Timber](https://www.upstatement.com/timber/) to structure its page templates and [Twig](https://twig.symfony.com/) for rendering them. [Timber's documentation](https://timber.github.io/docs/) is extensive and up to date, so be sure to get familiar with it.

As part of the [Twig Extension](#twig-extensions) the theme uses a Twig function in to render components into templates:

* `renderComponent(componentName, data)` renders a single component. [For example, in the `index.twig` template](https://github.com/NSC-Softwarewp/NSC-Software/tree/master/templates/index.twig).
...
The `functions.php` file for every component in the `./Components` folder is executed during the WordPress action `after_setup_theme`. [This is run from the `./functions.php` file of the theme.](https://github.com/NSC-Softwarewp/NSC-Software/tree/master/functions.php)
...
```php
namespace NSC-Software\Components\BlockWysiwyg;

function getACFLayout()
{
    return [
        'name' => 'blockWysiwyg',
        'label' => __('Block: Wysiwyg', 'NSC-Software'),
        'sub_fields' => [
            [
                'label' => __('Content', 'NSC-Software'),
                'name' => 'contentHtml',
                'type' => 'wysiwyg',
                'delay' => 0,
                'media_upload' => 0,
...
Field groups are needed to show registered fields in the WordPress back-end. All field groups are created in the `./inc/fieldGroups` folder. Two field groups exist by default: [`pageComponents.php`](https://github.com/NSC-Softwarewp/NSC-Software/tree/master/inc/fieldGroups/pageComponents.php) and [`postComponents.php`](https://github.com/NSC-Softwarewp/NSC-Software/tree/master/inc/fieldGroups/postComponents.php).
...
use NSC-Software\Components;

add_action('NSC-Software/afterRegisterComponents', function () {
    ACFComposer::registerFieldGroup([
        ...
        'fields' => [
            [
                'name' => 'pageComponents',
                'label' => __('Page Components', 'NSC-Software'),
                'type' => 'flexible_content',
                'button_label' => __('Add Component', 'NSC-Software'),
                'layouts' => [
...
Here we use the [ACF Field Group Composer](https://github.com/NSC-Softwarewp/acf-field-group-composer) plugin, which provides the advantage that all fields automatically get a unique key.
...
Other issues might come from NSC-Software not being able to determine the relative url of your uploads folder. If you have a non-standard WordPress folder structure, or if you use a plugin that manipulates `home_url` (for example, [WPML](https://wpml.org/)) this can cause problems when using `resizeDynamic`.

In this care try to set the relative upload path manually and refresh the permalink settings in the back-end:

```php
add_filter('NSC-Software/TimberDynamicResize/relativeUploadDir', function () {
...
To contribute, please use GitHub [issues](https://github.com/NSC-Softwarewp/NSC-Software/issues). Pull requests are accepted. Please also take a moment to read the [Contributing Guidelines](https://github.com/NSC-Softwarewp/guidelines/blob/master/CONTRIBUTING.md) and [Code of Conduct](https://github.com/NSC-Softwarewp/guidelines/blob/master/CODE_OF_CONDUCT.md).
