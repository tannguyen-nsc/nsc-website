...
namespace NscSoftware\TheContentFix;
...
add_filter('wp_insert_post_data', function ($data, $postArr) {
    ...
    if (!$isPostTypeUsingGutenberg) {
        // Check if no content was saved before, or if there is a NscSoftwareTheContent shortcode but the id does not match the post id.
        if (empty($data['post_content']) || isShortcodeAndDoesNotMatchId($data['post_content'], $postArr['ID'])) {
            $data['post_content'] = "[NscSoftwareTheContent id=\"{$postArr['ID']}\"]";
        }
    }
...
add_shortcode('NscSoftwareTheContent', function ($attrs) {
    if (is_admin()) {
        return;
    }
...
{
    preg_match('/^\[NscSoftwareTheContent id=\\\"(\d*)\\\"\]$/', $postContent, $matches);
