<?php

/**
 * Moves most relevant editor buttons to the first toolbar
 * and provides config for creating new toolbars, block formats, and style formats.
 * See the TinyMce documentation for more information: https://www.tiny.cloud/docs/
 */

namespace NscSoftware\TinyMce;

// First Toolbar.
add_filter('mce_buttons', function ($buttons) {
    $config = getConfig();
    if ($config && isset($config['toolbars'])) {
        $toolbars = $config['toolbars'];
        if (isset($toolbars['default']) && isset($toolbars['default'][0])) {
            return $toolbars['default'][0];
        }
    }
    return $buttons;
});

// Second Toolbar.
add_filter('mce_buttons_2', '__return_empty_array');

add_filter('tiny_mce_before_init', function ($init) {
    $config = getConfig();
    if ($config) {
        if (isset($config['blockformats'])) {
            $init['block_formats'] = getBlockFormats($config['blockformats']);
        }

        if (isset($config['styleformats'])) {
            // Send it to style_formats as true js array
            $init['style_formats'] = json_encode($config['styleformats']);
        }
    }
    return $init;
});

add_filter('acf/fields/wysiwyg/toolbars', function ($toolbars) {
    // Load Toolbars and parse them into TinyMCE.
    $config = getConfig();
    if ($config && !empty($config['toolbars'])) {
        $toolbars = array_map(function ($toolbar) {
            array_unshift($toolbar, []);
            return $toolbar;
        }, $config['toolbars']);
    }
    return $toolbars;
});

function getBlockFormats($blockFormats)
{
    if (!empty($blockFormats)) {
        $blockFormatStrings = array_map(
            function ($tag, $label) {
                return "{$label}={$tag}";
            },
            $blockFormats,
            array_keys($blockFormats)
        );
        return implode(';', $blockFormatStrings);
    }
    return '';
}

function getConfig()
{
    return [
        'blockformats' => [
            __('Paragraph', 'NscSoftware') => 'p',
            __('Heading 1', 'NscSoftware') => 'h1',
            __('Heading 2', 'NscSoftware') => 'h2',
            __('Heading 3', 'NscSoftware') => 'h3',
            __('Heading 4', 'NscSoftware') => 'h4',
            __('Heading 5', 'NscSoftware') => 'h5',
            __('Heading 6', 'NscSoftware') => 'h6'
        ],
        'styleformats' => [
            [
                'title' => __('Headings', 'NscSoftware'),
                'icon' => '',
                'items' => [
                    [
                        'title' => __('Heading 1', 'NscSoftware'),
                        'classes' => 'h1',
                        'selector' => '*'
                    ],
                    [
                        'title' => __('Heading 2', 'NscSoftware'),
                        'classes' => 'h2',
                        'selector' => '*'
                    ],
                    [
                        'title' => __('Heading 3', 'NscSoftware'),
                        'classes' => 'h3',
                        'selector' => '*'
                    ],
                    [
                        'title' => __('Heading 4', 'NscSoftware'),
                        'classes' => 'h4',
                        'selector' => '*'
                    ],
                    [
                        'title' => __('Heading 5', 'NscSoftware'),
                        'classes' => 'h5',
                        'selector' => '*'
                    ],
                    [
                        'title' => __('Heading 6', 'NscSoftware'),
                        'classes' => 'h6',
                        'selector' => '*'
                    ],
                ]
            ],
            [
                'title' => __('Buttons', 'NscSoftware'),
                'icon' => '',
                'items' => [
                    [
                        'title' => __('Button', 'NscSoftware'),
                        'classes' => 'button',
                        'selector' => 'a,button'
                    ],
                    [
                        'title' => __('Button Outlined', 'NscSoftware'),
                        'classes' => 'button--outlined',
                        'selector' => '.button'
                    ],
                    [
                        'title' => __('Button Text', 'NscSoftware'),
                        'classes' => 'button--text',
                        'selector' => '.button'
                    ],
                ]
            ],
            [
                'title' => __('Paragraph', 'NscSoftware'),
                'classes' => 'paragraph',
                'selector' => '*'
            ]
        ],
        'toolbars' => [
            'default' => [
                [
                    'formatselect',
                    'styleselect',
                    'bold',
                    'italic',
                    'strikethrough',
                    'blockquote',
                    '|',
                    'bullist',
                    'numlist',
                    '|',
                    'link',
                    'unlink',
                    '|',
                    'pastetext',
                    'removeformat',
                    '|',
                    'undo',
                    'redo',
                    'fullscreen'
                ]
            ],
            'basic' => [
                [
                    'bold',
                    'italic',
                    'strikethrough',
                    '|',
                    'link',
                    'unlink',
                    '|',
                    'undo',
                    'redo',
                    'fullscreen'
                ]
            ]
        ]
    ];
}
