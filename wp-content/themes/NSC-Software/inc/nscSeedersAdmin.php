...
/**
 * Ordered steps for “Run all seeders”: each script × (default language, then each Polylang locale).
 *
...
add_action('admin_enqueue_scripts', static function (string $hookSuffix): void {
    ...
    \wp_localize_script('nsc-seeders-admin', 'nscSeedersAdmin', [
        ...
        'runAllSteps' => build_run_all_sequencer_steps(),
        'i18n' => [
...
(function ($) {
  ...
  function setBusy(on) {
    $(".nsc-run-seeder, #nsc-seed-lang, #nsc-page-scope, #nsc-menu-rebuild, #nsc-run-all-seeders").prop(
      "disabled",
      !!on
    );
  ...
  $(document).on("click", "#nsc-run-all-seeders", function (e) {
    e.preventDefault();
    var steps = nscSeedersAdmin.runAllSteps || [];
    if (!steps.length) {
      notice("error", nscSeedersAdmin.i18n.badResponse);
      return;
    }
...
add_action('wp_ajax_nsc_run_seeder', static function (): void {
    ...
    if ($key === 'menus' && isset($_POST['menu_rebuild']) && (string) $_POST['menu_rebuild'] === '1') {
...
function render_page(): void
{
    ...
    if (\function_exists('pll_the_languages')) {
        ...
        if (\is_array($raw)) {
            foreach ($raw as $row) {
                if (\is_array($row) && !empty($row['slug'])) {
                    $slug = (string) $row['slug'];
                    $langs[$slug] = isset($row['name']) ? (string) $row['name'] : $slug;
                }
            }
        }
    }
    ...
    <div class="wrap">
        <h1>
            <?php echo \esc_html(\__('NSC HTTP seeders', 'NscSoftware')); ?>
            <span id="nsc-seeder-spinner" class="spinner" style="float:none;margin-top:4px"></span>
        </h1>
        <p class="description">
            <?php echo \esc_html(\__('Choose a Polylang language (or all), click a button, and wait. Results appear below without leaving this screen. “Default only” skips translation sync (no seed_lang).', 'NscSoftware')); ?>
        </p>
        ...
        <table class="form-table" role="presentation" style="max-width:720px;">
            <tr>
                ...
                <td>
                    <select id="nsc-seed-lang">
                        <option value=""><?php echo \esc_html(\__('Default only (no translation sync)', 'NscSoftware')); ?></option>
                        <option value="all"><?php echo \esc_html(\__('All non-default languages', 'NscSoftware')); ?></option>
            ...
            <tr>
                ...
                <td>
                    ...
                    <p class="description">
                        <?php echo \esc_html(\__('Applies only when you click “Pages”. Choose “All pages” to run the full page list. Matches create-nsc-pages.php page_scope.', 'NscSoftware')); ?>
                    </p>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    <p class="description">
                        <?php echo \esc_html(\__('If unchecked, only items from a previous NSC seed are replaced. Applies when you click “Menus”. “Run all seeders” always rebuilds menus on each menus step (this checkbox is ignored for that queue).', 'NscSoftware')); ?>
                    </p>
                </td>
            </tr>
        </table>
        ...
        <p class="description" style="max-width:960px;">
            <?php echo \esc_html(\__('Runs global options, pages, menus, blog posts, careers, and case studies in order: default language first, then each other Polylang language. Each step uses the same rules as the buttons below, except menus always run with rebuild (clear main + sitemap menus before seeding). “Pages” uses all pages. No Google API key: placeholders are (lang) in lowercase; legacy [LANG] is stripped before re-prefixing.', 'NscSoftware')); ?>
        </p>
        <p>
